 ...
 * One entry per enabled locale + `x-default` → `defaultLocale`
 * (`SiteSettings.defaultLocale`, already narrowed to the enabled set by the
 * caller via `pickDefaultLocale`; narrowed again here as a guard). Empty when
...
export function buildAlternates(
  ...
  const xDefault = enabled.includes(defaultLocale) ? defaultLocale : (enabled[0] as Locale);
  return [
    ...enabled.map((code) => ({ hreflang: code, href: abs(localizedPath(code, basePath)) })),
    { hreflang: 'x-default', href: abs(localizedPath(xDefault, basePath)) },
  ];
}
